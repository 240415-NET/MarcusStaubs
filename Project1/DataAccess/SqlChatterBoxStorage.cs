...
public class SqlChatterBoxStorage : IChatterBoxStorage
{
    public void InitializeChatterBox()
    {
        ...
        {
            ChatterBoxStorage myStorage = new();
            myStorage.InitializeChatterBox();
        }
        else
        {
            ...
            foreach (string chat in innList)
            {
                string cmdText = @"INSERT INTO Inn_Chatter (ChatText) VALUES (@chatText);";
                using SqlCommand cmd = new SqlCommand(cmdText, connection);
                cmd.Parameters.AddWithValue("@chatText", chat);
                cmd.ExecuteNonQuery();
            }
            ...
            newKillChatter.Add(kill1);
            newKillChatter.Add(kill2);
            newKillChatter.Add(kill3);
            newKillChatter.Add(kill4);
            newKillChatter.Add(kill5);
            newKillChatter.Add(kill6);
            newKillChatter.Add(kill7);
            newKillChatter.Add(kill8);
            newKillChatter.Add(kill9);
    ...
    public ChatterBox GetChatterBox()
    {
        ...
        else
        {
            ...
            using SqlConnection connection = new SqlConnection(connString);
            connection.Open();
            string cmdText = @"SELECT ChatText FROM Inn_Chatter;";
            using SqlCommand cmd = new SqlCommand(cmdText, connection);
            ...
            while (reader2.Read())
            {
                monsterTypes.Add(reader2.GetInt32(0));
            }
            foreach (int monsterID in monsterTypes)
            {
                KillChatter newKillChatter = new();
                newKillChatter.monsterType = monsterID;
                string cmdText3 = @"SELECT ChatText FROM Kill_Chatter WHERE MonsterID = @monsterType;";
                using SqlCommand cmd3 = new SqlCommand(cmdText3, connection);
                cmd3.Parameters.AddWithValue("@monsterType", monsterID);
